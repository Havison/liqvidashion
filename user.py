...
async def message_bybit_binance(tg_id, symbol, liquidation_type, price):
    try:
        ...
        await bot.send_message(
            chat_id=tg_id,
            text=f'{sml[liquidation_type]}<b>{symbol[0:-4]}</b>, сумма ликвидации: {price} USDT\n'
                 f'<a href=\"{bybit}\">ByBit</a> | <a href=\"{coinglass}\">CoinGlass</a> | <a href=\"{binance}\">Binance</a>',
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True
        )
        logger.info(f"Сообщение отправлено в Telegram для {symbol} с суммой ликвидации {price} USDT.")
